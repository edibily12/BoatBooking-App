...
if (isset($_SESSION['verified']) && $_SESSION['verified'] === false) {
    LogsController::create("trying to access login page through url while not verified");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && $_POST["action"] === 'login') {
    $recaptchaSecret = "6LfTmQIqAAAAALdfsOfhABHoTLZyXbwVoM5R7rm1";
    ...
    <section class="min-h-screen flex items-stretch text-white ">
        ...
        <div class="lg:w-1/2 w-full flex items-center justify-center text-center md:px-16 px-0 z-0" style="background-color: #161616;">
            ...
            <div class="w-full py-6 z-20">
                ...
                <form id="login-form" action="" method="post" class="sm:w-2/3 w-full px-4 lg:px-0 mx-auto">
                    <input type="hidden" value="login" name="action">
                    <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">

                    <div class="pb-2 pt-4">
                        <input type="email" name="email" value="<?= $values['email'] ?>" id="email" placeholder="Email" class="block w-full p-4 text-lg rounded-sm bg-black">
                    </div>

                    <div class="pb-2 pt-4">
                        <input class="block w-full p-4 text-lg rounded-sm bg-black" type="password" name="password" value="<?= $values['password'] ?>" id="password" placeholder="Password">
                    </div>

                    <div class="g-recaptcha w-full" data-sitekey="6LfTmQIqAAAAAB5i99ojfUGtToN5aD8ft5PbJET6"></div>
                    ...
                    <div class="px-4 pb-2 pt-4">
                        <button id="login-btn" type="submit" class="uppercase block w-full p-4 text-lg rounded-full bg-indigo-500 hover:bg-indigo-600 focus:outline-none">LOGIN</button>
                    </div>
                </form>
            </div>
        </div>
